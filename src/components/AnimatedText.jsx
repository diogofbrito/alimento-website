...
import { h1SlideUp, ButtonSlideUp, pFadeIn, pFadeInAfterH1 } from './animations/variants.js';
...
export function AnimatedH1({ children, className = '' }) {
	return (
		<motion.div className={`overflow-hidden inline-block ${className}`} initial='hidden' whileInView='show' viewport={{ once: true, amount: 0.2 }}>
			<motion.h1 variants={h1SlideUp} className='inline-block will-change-transform '  transition={{ duration: 0.1, ease: 'easeInOut' }}>
				{children}
			</motion.h1>
		</motion.div>
	);
